...
class IDS {
        ...
        try {
            ...
            while (scanner.hasNextLine()) {
                if (lineCounter == 0) {
                    ...
                    if (minimum > maximum) {
                        System.err
                                .println("Error. Event " + eventName + ", minimum value is larger than maximum value.");
                        return false;
                    }
     ...
     * 6. The number of events monitored which is specified at the first line of the
     * events/stats file, does not tally with the actual number of records in the
     * file.
     */
    private static void checkFileInconsistency() {
        ...
        if (eventList.size() != statList.size()) {
            System.out.println("Error. Number of events differ by "
                    + (eventList.size() > statList.size() ? eventList.size() - statList.size()
                            : statList.size() - eventList.size()));
            ok = false;
        } else {
            ...
            try {
                ...
                if (size != eventList.size() || size_2 != statList.size()) {
                    System.out.println(
                            "Number of events specified in the file does not tally with the actual number of events in the file");
                    ok = false;
                }
                fr.close();
                fr_2.close();
            ...
            for (int i = 0; i < eventList.size(); i++) {
                ...
                if (eventList.get(i).getMaximum() < statList.get(i).getMean()) {
                    System.out.println("Error. Event " + statList.get(i).getEventName()
                            + " mean value is greater than maximum value");
                    ok = false;
                }
                ...
                if (eventList.get(i).getMinimum() > eventList.get(i).getMaximum()) {
                    System.out.println("Error. Continuous Event " + eventList.get(i).getEventName()
                ...
                if (eventList.get(i).getMin() > eventList.get(i).getMax()) {
                    System.out.println("Error. Discrete Event " + eventList.get(i).getEventName()
                            + " minimum value is greater than maximum value");
                    ok = false;
                }
                ...
                if (eventList.get(i).getWeight() <= 0) {
                    System.out.println(
                            "Error. Event " + eventList.get(i).getEventName() + " weight is less than or equal to 0");
                    ok = false;
                }
            }
        }
    ...
    private static void discreteEventSimulation(String name, int minimum, int maximum, double mean,
            double standardDeviation, boolean isAlertPhase, int dayCounter, int serverSecret, FileWriter fw) {
    ...
    private static void continuousEventSimulation(String name, double minimum, double maximum, double mean,
            double standardDeviation, boolean isAlertPhase, int dayCounter, int serverSecret, FileWriter fw) {
        Random rand = new Random();
        double tempMin = 0;
        try {
                ...
                for (Event e : eventList) {
                    for (Stats s : statList) {
                        if (e.getEventName().equals(s.getEventName())) {
                            ...
                            if (e.isDiscrete()) {
                                discreteEventSimulation(name, e.getMin(), e.getMax(), mean, standardDeviation,
                                        isAlertPhase, i, serverSecret, fw);
                            } else {
                                continuousEventSimulation(name, e.getMinimum(), e.getMaximum(), mean, standardDeviation,
                                        isAlertPhase, i, serverSecret, fw);
                            }
                        }
                    }
